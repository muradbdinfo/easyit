...
const props = defineProps({ faq: Object, services: Array })
const form = useForm({ service_id: props.faq.service_id || '', question: props.faq.question, answer: props.faq.answer, sort_order: props.faq.sort_order, is_active: props.faq.is_active })
const submit = () => form.put(`/admin/faqs/${props.faq.id}`)
</script>
<template>
    <AdminLayout>
        <div class="flex items-center justify-between mb-6"><h1 class="text-2xl font-bold text-gray-900">Edit FAQ</h1><Link href="/admin/faqs" class="text-sm text-gray-600 hover:text-gray-900">← Back</Link></div>
        <form @submit.prevent="submit" class="bg-white shadow rounded-lg p-6 max-w-3xl space-y-4">
            <FormSelect v-model="form.service_id" label="Related Service" :options="services" value-key="id" label-key="title" placeholder="General" />
            <FormInput v-model="form.question" label="Question" required />
            <FormTextarea v-model="form.answer" label="Answer" :rows="5" required />
